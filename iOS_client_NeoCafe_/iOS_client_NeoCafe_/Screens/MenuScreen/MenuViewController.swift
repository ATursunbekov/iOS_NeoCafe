    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if collectionView == menuView.categoryCollectionView {
            ...
            if indexPath.row == viewModel.selectedIndex {
                cell.selected()
                viewModel.getCategoryProducts(category: viewModel.getCategoryName(index: indexPath.row))
            } else {
                cell.deselect()
            }
            return cell
        } else {
            let cell: MenuCollectionViewCell = collectionView.dequeue(for: indexPath)
